...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？
   ...
   答：一开始由2个叶子组成二叉树,产生1新结点,也就是说做了一次计算，增加了一个节点，然后每次都是如此，会有n-1次运算，从而产生了根节点，所以会有一共第1次必定是2个叶子组成二叉树,产生1新结点,也就是说做了一次计算，增加了一个节点，重复下去，会有n-1次运算，从而产生了根节点，所以会有一共2n-1个节点。

2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？

   答：修改if(str[i] == (char)(0x41 + j))为if(str[i] == (char)(0x61 + j))使程序从小写字母开始遍历，如要同时扫描大小写，则还是从0x41开始，但修改arr_big数组的长度为54，且for(int j = 0; j < 27; j++)也对应改成for(int j = 0; j < 60; j++) ，使程序能扫描完全部字母。 
