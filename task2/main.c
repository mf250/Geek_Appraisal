...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i]>0)
        {
            arr_small[count2++] = arr_big[i];	 //将arr_big中的权重值放进arr_small对应字母一行中 
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //如果i节点没有左右节点，即i节点为叶节点 
        {
...
void huffmanTree(int w[],int huffman[][4],int n)
{
    ...
    //使Huffman的前n行代表叶节点 ,且每个叶节点被赋予对应的权重值 
    for(int i = 0; i < n; i++)
    {
        huffman[i][3] = w[i];
    }
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*补充判断模块注释（说明作用）*/ //筛选出非叶节点，不进行进一步计算 
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  /*补充判断模块注释（说明作用）*/ //选出权值小于min的叶节点，进行下面的运算 
        {
            min = huffman[i][3];
            ii[0] = i;
        }
